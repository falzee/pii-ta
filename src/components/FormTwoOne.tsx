    ...
    const fetchFaipData = async () => {
      try {
        ...
        if (token) {
          ...
          const response = await axios.get(`http://localhost:8000/form-penilaian/mhs?uid=${userId}&ft=ii1`,config)
    ...
    const onFinish = async (values: any) => { //fungsi submit form //NEED API POST
      ...
      try{
        ...
        //   hubungan: string;
          ...
          const response = await axios.patch(`http://localhost:8000/form-penilaian/mhs?uid=${userId}&pid=${formId}&ft=ii1`,formData,config);
...
//kolom tabel
    const columns: ColumnsType<TableRow>= [
        ...
        },
        {
            ...
            key: 'alamat',
            render: (text: string, record: TableRow) => (
              <Form.Item name={`alamat${record.key}`} initialValue={text} style={{width:'150px'}}>
                <TextArea rows={4} />
              </Form.Item>
            ),
          },
