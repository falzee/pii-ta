    ...
    const fetchFaipData = async () => {
      try {
        ...
        if (token) {
          ...
          const response = await axios.get(`http://192.168.195.241:8000/form-penilaian/mhs?uid=${userId}&ft=v2`,config)
          const userData = response.data;
          setDataSource(userData.data.form_v_dua)
          const newSelectedChoices: { [key: string]: string[] } = {};
          userData.data.form_v_dua.forEach((item: any) => {
    ...
    const onFinish = async (values: any) => { //fungsi submit form //NEED API POST
      ...
      try{
        ...
        if (token) {
          ...
          const response = await axios.patch(`http://192.168.195.241:8000/form-penilaian/mhs?uid=${userId}&pid=${formId}&ft=v2`,formData,config);
...
//kolom tabel
    const columns: ColumnsType<TableRow>= [
        ...
        {
          title: 'Judul Makalah/Tulisan',
          dataIndex: 'judulMakalah',
          key: 'judulMakalah',
          render: (text: string, record: TableRow) => (
            <Form.Item name={`judulMakalah${record.key}`} initialValue={text} style={{width:'200px'}}>
              <TextArea rows={4} />
            </Form.Item>
          ),
        },
        ...
        },
        {
          ...
          {
            title: 'Tingkat Seminar/Lokakarya',
          ...
          {
            title: 'Uraian Singkat Tulisan/Makalah yang Disajikan',
    ...
    <div>
        <div className='container-form'>
            <h3 className='headerform' style={{marginBottom:'10px'}}>V.2 Makalah/Tulisan Yang Disajikan Dalam Seminar/Lokakarya Keinsinyuran <span style={{color:'#6b7aa1'}}>(W4)</span></h3>
