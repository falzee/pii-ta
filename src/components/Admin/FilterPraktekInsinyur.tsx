...
const FilterPraktekInsinyur: React.FC = () => {
    ...
    const fetchUserData = async () => {
        ...
        if (token) {
            ...
            const responseFilter = await axios.get(`/form-tools/faip-form/form-rule?uid=${userId}&ft=praktik-keinsinyuran`,config);
            // Validate selected values against allOptions
            const filterData = responseFilter.data.data.mk_praktik_keinsinyuran
    ...
    const onFinish = async () => { //fungsi submit form //NEED API POST
        ...
        if (token) {
            ...
            await axios.patch(`/form-tools/faip-form/filtering?uid=${userId}&ft=praktik-keinsinyuran`,formData,config);
        } else {
            console.error('User not found');
        }
        }catch(error){
...
<div>
    ...
    <h4 style={{margin:"0 0 10px"}}>CPMK 1. Mahasiswa mampu memahami tentang filosofi keinsinyuran dan arah perkembangan keinsinyuran.</h4>
    <p>Tampilan Pilihan: <span style={{color:"blue",fontWeight:"bold"}}>{selectedChoicesOne}</span></p>
        ...
        <h4 style={{margin:"10px 0"}}>CPMK 2. Mahasiswa mampu memahami tentang sistem industri atau sistem keteknikan (“engineering”).</h4>
        ...
        <h4 style={{margin:"10px 0"}}>CPMK 3. Mahasiswa mampu melakukan praktik keinsinyuran dan mampu memecahkan permasalahan di tempat kerja praktik.</h4>
        ...
        <h4 style={{margin:"10px 0"}}>CPMK 4. Mahasiswa mampu membuat laporan keinsinyuran secara lengkap.</h4>
        ...
        <h4 style={{margin:"10px 0"}}>CPMK 5. Mahasiswa mampu mempresentasikan dan mengkomunikasikan hasil kerja keinsinyurannya.</h4>
...
export default FilterPraktekInsinyur;
