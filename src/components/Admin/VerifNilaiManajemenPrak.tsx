...
interface ListItem {
  id_dosen: string;
  dosen_name: string;
  mk_manajemen_praktik: SomeObject; // Nested object
}
...
const VerifNilaiManajemenPrak: React.FC = () => {
//kumpulan state
    ...
    const fetchFaipData = async () => {
      try {
        ...
        if (token) {
          ...
          const responseGrading = await axios.get(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&sid=${nimMhs}&ft=manajemen-praktik`,config);
          ...
          if (adminGrading && adminGrading.nilai_Verif_manajemen_praktik.nilai_cpmk){
            setValues(adminGrading.nilai_Verif_manajemen_praktik.nilai_cpmk);
          }

          const validGrading = dsnGrading.filter(
            (item: any) => Object.keys(item.mk_manajemen_praktik).length > 0
          );
    ...
    const onFinish = async () => { //fungsi submit form //NEED API POST
      try{
        ...
        if (token) {
          ...
          await axios.patch(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&ft=manajemen-praktik`,formData,config);
    ...
    const handleInputChange = (key: string, value: number | null) => {
      ...
      const calculateAverage = (key: keyof SubObject) => {
        const total = dsnData.reduce(
          (sum, item) => sum + (item.mk_manajemen_praktik.nilai_cpmk[key] || 0),
          0
        );
...
return (
    ...
    <div style={{marginBottom:'2rem'}}>
        ...
        {/* CPMK 1 */}
        <h4>CPMK 1. Mahasiswa mampu menjelaskan teori dan konsep tata Kelola proyek.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_manajemen_praktik.nilai_cpmk.cpmk_1 ?? 0} </p>
            </div>
        ))}
        ...
        {/* CPMK 2 */}
        <h4>CPMK 2. Mahasiswa mampu menerapkan etika profesi dan kode etik keinsinyuran.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_manajemen_praktik.nilai_cpmk.cpmk_2 ?? 0} </p>
            </div>
        ))}
        ...
        {/* CPMK 3 */}
        <h4>CPMK 3. Mahasiswa mampu menerapkan ketrampilan berorganisasi dan berkomunikasi.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_manajemen_praktik.nilai_cpmk.cpmk_3 ?? 0} </p>
            </div>
        ))}
        ...
        {/* CPMK 4 */}
        <h4>CPMK 4. Mahasiswa mampu menerapkan teknologi informasi yang dinamis dalam menjalankan praktik keinsinyuran.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_manajemen_praktik.nilai_cpmk.cpmk_4 ?? 0} </p>
            </div>
        ))}
...
export default VerifNilaiManajemenPrak;
