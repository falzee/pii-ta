...
interface ListItem {
  id_dosen: string;
  dosen_name: string;
  mk_profesionalisme: SomeObject; // Nested object
}
...
const VerifNilaiProfesionalisme: React.FC = () => {
//kumpulan state
    ...
    const fetchFaipData = async () => {
      try {
        ...
        if (token) {
          ...
          const responseGrading = await axios.get(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&sid=${nimMhs}&ft=profesionalisme`,config);
          ...
          if (adminGrading && adminGrading.nilai_Verif_profesionalisme.nilai_cpmk){
            setValues(adminGrading.nilai_Verif_profesionalisme.nilai_cpmk);
          }

          const validGrading = dsnGrading.filter(
            (item: any) => Object.keys(item.mk_profesionalisme).length > 0
          );
    ...
    const onFinish = async () => { //fungsi submit form //NEED API POST
      try{
        ...
        if (token) {
          ...
          await axios.patch(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&ft=profesionalisme`,formData,config);
    ...
    const handleInputChange = (key: string, value: number | null) => {
      ...
      const calculateAverage = (key: keyof SubObject) => {
        const total = dsnData.reduce(
          (sum, item) => sum + (item.mk_profesionalisme.nilai_cpmk[key] || 0),
          0
        );
    ...
    <div style={{marginBottom:'2rem'}}>
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_1 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_2 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_3 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_4 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_5 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_6 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_7 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_8 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_profesionalisme.nilai_cpmk.cpmk_9 ?? 0} </p>
            </div>
        ))}
        <p>Rata-rata nilai CPMK 9 : <span style={{color:'blue'}}>{Math.round(calculateAverage("cpmk_9"))}</span></p>
        <div style={{display:'flex',flexDirection:'row'}}>
            <p style={{margin:'5px 0'}}>Verifikasi nilai CPMK 9 :</p>
                <InputNumber style={{width:'75px',margin:'0 5px'}} 
                min={0} 
                max={100}  
                value={values["cpmk_9"]}
        ...
        <Button type="primary" onClick={() => handleCopyAverage("cpmk_9")}><CopyOutlined /> Kopi nilai CPMK 9</Button>
        <Divider style={{margin:'10px 0'}} />
...
export default VerifNilaiProfesionalisme;
