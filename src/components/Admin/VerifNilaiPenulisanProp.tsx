...
interface ListItem {
  id_dosen: string;
  dosen_name: string;
  mk_penulisan_proposal_studi_kasus: SomeObject; // Nested object
}
...
const VerifNilaiPenulisanProp: React.FC = () => {
//kumpulan state
    ...
    const fetchFaipData = async () => {
      try {
        ...
        if (token) {
          ...
          const responseGrading = await axios.get(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&sid=${nimMhs}&ft=penulisan-proposal-sk`,config);
          ...
          if (adminGrading && adminGrading.nilai_Verif_penulisan_proposal_studi_kasus.nilai_cpmk){
            setValues(adminGrading.nilai_Verif_penulisan_proposal_studi_kasus.nilai_cpmk);
          }

          const validGrading = dsnGrading.filter(
            (item: any) => Object.keys(item.mk_penulisan_proposal_studi_kasus).length > 0
          );
    ...
    const onFinish = async () => { //fungsi submit form //NEED API POST
      try{
        ...
        if (token) {
          ...
          await axios.patch(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&ft=penulisan-proposal-sk`,formData,config);
    ...
    const handleInputChange = (key: string, value: number | null) => {
      ...
      const calculateAverage = (key: keyof SubObject) => {
        const total = dsnData.reduce(
          (sum, item) => sum + (item.mk_penulisan_proposal_studi_kasus.nilai_cpmk[key] || 0),
          0
        );
    ...
    <div style={{marginBottom:'2rem'}}>
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_penulisan_proposal_studi_kasus.nilai_cpmk.cpmk_1 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_penulisan_proposal_studi_kasus.nilai_cpmk.cpmk_2 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_penulisan_proposal_studi_kasus.nilai_cpmk.cpmk_3 ?? 0} </p>
            </div>
        ))}
        ...
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_penulisan_proposal_studi_kasus.nilai_cpmk.cpmk_4 ?? 0} </p>
            </div>
        ))}
...
export default VerifNilaiPenulisanProp;
