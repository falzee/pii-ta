...
interface ListItem {
  id_dosen: string;
  dosen_name: string;
  mk_k3lh: SomeObject; // Nested object
}
...
const VerifNilaiKtiga: React.FC = () => {
//kumpulan state
    ...
    const fetchFaipData = async () => {
      try {
        ...
        if (token) {
          ...
          const responseGrading = await axios.get(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&sid=${nimMhs}&ft=k3lh`,config);
          ...
          if (adminGrading && adminGrading.nilai_Verif_k3lh.nilai_cpmk){
            setValues(adminGrading.nilai_Verif_k3lh.nilai_cpmk);
          }

          const validGrading = dsnGrading.filter(
            (item: any) => Object.keys(item.mk_k3lh).length > 0
          );
    ...
    const onFinish = async () => { //fungsi submit form //NEED API POST
      try{
        ...
        if (token) {
          ...
          await axios.patch(`/form-tools/verif-grade/grading-info?uid=${userId}&pid=${formIdA}&ft=k3lh`,formData,config);
    ...
    const handleInputChange = (key: string, value: number | null) => {
      ...
      const calculateAverage = (key: keyof SubObject) => {
        const total = dsnData.reduce(
          (sum, item) => sum + (item.mk_k3lh.nilai_cpmk[key] || 0),
          0
        );
...
return (
    ...
    <div style={{marginBottom:'2rem'}}>
        ...
        {/* CPMK 1 */}
        <h4>CPMK 1. Mahasiswa mampu menjelaskan prinsip-prinsip dasar K3 dan peraturan-peraturan yang berlaku di tempat kerja.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_k3lh.nilai_cpmk.cpmk_1 ?? 0} </p>
            </div>
        ))}
        ...
        {/* CPMK 2 */}
        <h4>CPMK 2. Mahasiswa mampu mengidentifikasi potensi bahaya dan risiko di lingkungan kerja serta mengambil tindakan pencegahan yang sesuai.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_k3lh.nilai_cpmk.cpmk_2 ?? 0} </p>
            </div>
        ))}
        ...
        {/* CPMK 3 */}
        <h4>CPMK 3. Mampu menyusun prosedur keselamatan kerja yang tepat dan cara mengatasi keadaan darurat, seperti kebakaran atau kecelakaan.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_k3lh.nilai_cpmk.cpmk_3 ?? 0} </p>
            </div>
        ))}
        ...
        {/* CPMK 4 */}
        <h4>CPMK 4. Mampu melakukan inspeksi dan perawatan peralatan kerja untuk memastikan kondisi kerja yang aman.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_k3lh.nilai_cpmk.cpmk_4 ?? 0} </p>
            </div>
        ))}
        ...
        {/* CPMK 5 */}
        <h4>CPMK 5. Mampu membuat pelaporan kecelakaan dan insiden, serta partisipasi aktif dalam program-program K3 di tempat kerja.</h4>
        {dsnData.map((item) => (
            <div key={item.id_dosen}>
              <p> - Nilai dari {item.dosen_name} : {item.mk_k3lh.nilai_cpmk.cpmk_5 ?? 0} </p>
            </div>
        ))}
...
export default VerifNilaiKtiga;
