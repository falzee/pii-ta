...
const NewFormNilaiPraktikInsinyur: React.FC = () => {
//kumpulan state
      ...
      const fetchFaipData = async () => {
        try {
          ...
          if (token) {
            ...
            const responseFormFilter = await axios.get(`/form-penilaian/dsn/form-filter?uid=${userId}&ft=praktik-keinsinyuran`,config);
            
            const formFilter = responseFormFilter.data.data.mk_praktik_keinsinyuran;
            setAllFilter(formFilter);
            
            const responseMhsGrading = await axios.get(`/form-penilaian/dsn/update-nilai?uid=${userId}&pid=${formIdD}&ft=praktik-keinsinyuran`,config);
            const mhsGrading = responseMhsGrading.data.data

            if (mhsGrading && mhsGrading.mk_praktik_keinsinyuran.nilai_cpmk){
              setValues(mhsGrading.mk_praktik_keinsinyuran.nilai_cpmk);
            }
        } else {
    ...
    const onFinish = async () => { //fungsi submit form //NEED API POST
      try{
        ...
        if (token) {
          ...
          await axios.patch(`/form-penilaian/dsn/update-nilai?uid=${userId}&pid=${formIdD}&ft=praktik-keinsinyuran`,formData,config);
...
export default NewFormNilaiPraktikInsinyur;
