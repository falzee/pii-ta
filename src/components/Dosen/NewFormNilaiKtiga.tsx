...
const NewFormNilaiKtiga: React.FC = () => {
//kumpulan state
      ...
      const fetchFaipData = async () => {
        try {
          ...
          if (token) {
            ...
            const responseFormFilter = await axios.get(`/form-penilaian/dsn/form-filter?uid=${userId}&ft=k3lh`,config);
            
            const formFilter = responseFormFilter.data.data.mk_k3lh;
            setAllFilter(formFilter);
            
            const responseMhsGrading = await axios.get(`/form-penilaian/dsn/update-nilai?uid=${userId}&pid=${formIdD}&ft=k3lh`,config);
            const mhsGrading = responseMhsGrading.data.data

            if (mhsGrading && mhsGrading.mk_k3lh.nilai_cpmk){
              setValues(mhsGrading.mk_k3lh.nilai_cpmk);
            }
        } else {
...
export default NewFormNilaiKtiga;
