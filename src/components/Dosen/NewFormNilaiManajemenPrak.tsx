...
const NewFormNilaiManajemenPrak: React.FC = () => {
//kumpulan state
      ...
      const fetchFaipData = async () => {
        try {
          ...
          if (token) {
            ...
            const responseFormFilter = await axios.get(`/form-penilaian/dsn/form-filter?uid=${userId}&ft=manajemen-praktik`,config);
            
            const formFilter = responseFormFilter.data.data.mk_manajemen_praktik;
            setAllFilter(formFilter);
            
            const responseMhsGrading = await axios.get(`/form-penilaian/dsn/update-nilai?uid=${userId}&pid=${formIdD}&ft=manajemen-praktik`,config);
            const mhsGrading = responseMhsGrading.data.data

            if (mhsGrading && mhsGrading.mk_manajemen_praktik.nilai_cpmk){
              setValues(mhsGrading.mk_manajemen_praktik.nilai_cpmk);
            }
        } else {
    ...
    const onFinish = async () => { //fungsi submit form //NEED API POST
      try{
        ...
        if (token) {
          ...
          await axios.patch(`/form-penilaian/dsn/update-nilai?uid=${userId}&pid=${formIdD}&ft=manajemen-praktik`,formData,config);
...
export default NewFormNilaiManajemenPrak;
